...
export class ASTHelper {
        ...
        for (const token of tokens) {
            if (token.type === 'tag-start') {
                ...
                if (!isStandardDOMTag(token.value)) {
                    const component = this.controller.components[token.value]
                    element = this.controller.VDOMHelper.createVComponent(token.value, {}, [], component.style)
                    element.children = this.parse(this.lex(component.template)).children;
                } else {
                    element = this.controller.VDOMHelper.createVNode(token.value)
                }
    ...
    processPropsComponent(node:VNode, isFirstRoot = true, parentComponent?: VNode, grandParentComponent?: VNode):VNode {
        ...
        if (!isFirstRoot && newNode.type === 'component' && this.controller.components[newNode.tagName]) {
            currentComponentInstance.value = node as VComponentNode
